...
class StockData() :
    ...
    def filterDfs(self, filteringDf, filteredDf) :      # filetingDf = 필터하는 df / filteredDf = 필터 당하는 df
    ...
    def differenceDf(self, oldDay, newDay) :
        oldDf = self.createTickerDf(oldDay)     # 어제의 ticker df
        newDf = self.createTickerDf(newDay)     # 오늘의 ticker df
        delistedDf = self.filterDfs(newDf, oldDf)   # 어제를 마지막으로 오늘은 존재하지 않는 ticker df
        listedDf = self.filterDfs(oldDf, newDf) # 어제는 없었지만 오늘은 존재하는 ticker df
        return delistedDf, listedDf    

    def creatingTickerInfoDf(self, day) :     # 가장 첫날에만 사용하는 함수(day = db의 가장 첫날)
        tickers = stock.get_market_ticker_list(day, market="KOSPI")  # day에 해당하는 일자에 KOSPI에 존재하는 모든 ticker를 호출함 
        df = pd.DataFrame(tickers, columns=['ticker'])
        df = df.assign(name = df['ticker'].apply(lambda x: stock.get_market_ticker_name(x)))     # 존재하는 ticker에 대해 loop를 돌며 해당 ticker의 상호명을 새로운 열에 추가
        df = df.assign(key=df.apply(lambda x: x.loc['ticker']+x.loc['name'], axis=1))    # ticker와 name을 합쳐서 key값을 만듬(해당 ticker가 거리가 먼 서로 다른 일자에 다른 상호명으로 사용되었을 경우를 대비)
    ...
    def updateTickerInfoDf(self, stockListDf, delistedDf, listedDf, oldDay, newDay) :   #   stockListDf는 oldDay에 완성된 ticker df
    ...
    def getDayList(self, startDay, endDay):   # 해당 기간동안 존재하는 모든 일자를 리스트로 반환함
        # Day List
    ...
    def updateDfByAllStock(self, oldDf, date) :       # stockPriceDb에 해당하는 함수
        date = date
        df = oldDf
    ...
    def getPeriodStockListDf(self, start, end) :
        stockListDf = pd.read_parquet(self.folderPath/'stockListDB.parquet')
    ...
    def createDb(self, kind, start, end) :   # kind = 'stockList', 'stockPrice', 'nOfshares', 'fundermental' 중 하나, start/end는 '20100101', '20211008' 같은 양식

        if kind == 'stockList' :
            dayList = self.getDayList(start, end)

            for k in tqdm(range(0, len(dayList))) :
                if k == 0 :
                    stockListDf = self.creatingTickerInfoDf(dayList[k])
                else :
                    oldDay = dayList[k-1]
                    newDay = dayList[k]
                    delistedDf, listedDf = self.differenceDf(oldDay, newDay)
                    stockListDf = self.updateTickerInfoDf(stockListDf, delistedDf, listedDf, oldDay, newDay)
            ...
            for day in tqdm(dayList) :
                stockPriceDB = self.updateDfByAllStock(stockPriceDB, day)
